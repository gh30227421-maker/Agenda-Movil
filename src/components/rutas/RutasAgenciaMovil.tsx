...
export default function RutasAgenciaMovil({ selectedMonths = [] }: { selectedMonths?: string[] }) {
  ...
  const events = useMemo(() => {
    return allEvents.filter(e => 
      (e.type === 'Agencia Móvil' || (isCombinado && e.type === 'Red de Agencias')) &&
      (selectedMonths.length === 0 || (e.startDate && selectedMonths.some(m => e.startDate?.startsWith(m))))
    );
  }, [allEvents, isCombinado, selectedMonths]);
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        ...
        if (data) {
          const aVideos = data
            .filter(f => f.name.startsWith('agencia-movil-video') && f.name.endsWith('.mp4'))
            .map(f => supabase.storage.from('event_photos').getPublicUrl(f.name).data.publicUrl);
          
          if (aVideos.length > 0) {
            setPlaylist(aVideos);
          } else {
            setPlaylist([supabase.storage.from('event_photos').getPublicUrl('agencia-movil-video.mp4').data.publicUrl]);
          }
        }
      } catch (e) {
        setPlaylist([supabase.storage.from('event_photos').getPublicUrl('agencia-movil-video.mp4').data.publicUrl]);
              ...
              <div className="absolute top-0 left-4 md:top-[-20px] z-50 bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-200 p-4 w-[320px] animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto pr-1 hide-scrollbar">
                  {events.filter(e => normalizeStateName(e.estadoOperativo || e.state) === drilldownState).map(ev => (
                     <button 
                       key={ev.id}
                       onClick={() => setDrilldownEventId(drilldownEventId === ev.id ? null : ev.id)}
            ...
            <div className="w-full h-auto rounded-2xl overflow-hidden shadow-[0_15px_40px_rgba(0,32,91,0.1)] border border-slate-200 bg-slate-900/40 backdrop-blur-sm group pointer-events-auto relative">
              ...
              <div className="absolute top-3 right-3 z-20 bg-[#00205B]/80 backdrop-blur border border-white/20 rounded-full px-2 py-1 flex items-center gap-1.5 shadow-md transition-all">
                 ...
                 <span className="text-[9px] font-bold uppercase tracking-wider text-white">
                   Agencia Móvil {playlist.length > 1 ? `(${currentVideoIndex + 1}/${playlist.length})` : ''}
                 </span>
              </div>
            </div>
          </div>
